(defpackage :cl-lsp.lem-lsp-utils/protocol-generator
  ...
  (:export :deploy))
(in-package :cl-lsp.lem-lsp-utils/protocol-generator)

(cl-package-locks:lock-package :cl-lsp.lem-lsp-utils/protocol-generator)
...
(defun ensure-protocol-package ()
  (or (find-package :cl-lsp.lem-lsp-utils/protocol)
      (make-package :cl-lsp.lem-lsp-utils/protocol :use '())))
  ...
  (with-open-file (out out-file
    ...
    (let* ((*export-list* '())
      ...
      (format out ";;; Code generated by ~A based on ~A; DO NOT EDIT.~%"
              :cl-lsp.lem-lsp-utils/protocol-generator
              (make-pathname :name (pathname-name spec-file)
                             :type (pathname-type spec-file)))
      (pprint `(defpackage :cl-lsp.lem-lsp-utils/protocol
                 (:import-from :cl-lsp.lem-lsp-utils/json)
                 (:import-from :cl-lsp.lem-lsp-utils/type)
                 (:export ,@(mapcar #'make-keyword *export-list*)))
              out)
      (pprint '(in-package :cl-lsp.lem-lsp-utils/protocol) out)
      (format out "~&~A" "(cl-package-locks:lock-package :cl-lsp.lem-lsp-utils/protocol)")
      (write-string body-text out)))
  (values))

(defun deploy ()
  (generate (asdf:system-relative-pathname :cl-lsp.lem-lsp-utils "specification/specification-3-15.md")
            (asdf:system-relative-pathname :cl-lsp.lem-lsp-utils "protocol.lisp")))
